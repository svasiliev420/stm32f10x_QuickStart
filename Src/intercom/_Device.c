...
/////////////////////////
    ...
    uint8_t  device_bufferRx[32];
    
    void INTERCOM_Send(void *src)
    {
        memcpy(&device_bufferTx, src, 32);
        
        LL_DMA_SetDataLength (_xdma, _xdma_chTX, 32);
        LL_DMA_EnableChannel (_xdma, _xdma_chTX);
    }
    
    void INTERCOM_Setup()
    {
        memset(device_bufferTx, 0x00, 32);
        memset(device_bufferRx, 0x00, 32);
////////
// GPIO
////////
